...
const Onboarding = () => {
  ...
  useEffect(() => {
    if (adhar) {
      getAdharUrl(adhar[adhar.length - 1]);

    }
  }, [adhar]);
  useEffect(() => {
    if (report) {
      getImageURL(report[report.length - 1]);
      localStorage.setItem("pdf",URL.createObjectURL(report[report.length-1]))
    }
  }, [report]);
  return (
    <>
      ...
      <div className="mx-10 my-4">
        ...
        <form className="flex  my-4 text-sm" onSubmit={submitHandler}>
          <div className="flex flex-col w-[30%]">
            ...
            <label className="my-1 text-sm">Medical Condition</label>
            <textarea
          ...
          <div className="w-[50%] mx-10">
            ...
            <button className="my-4 px-6 py-2 rounded-md text-white bg-blue-600">Submit</button>
          </div>
